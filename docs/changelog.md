...
[**Release:**](https://github.com/KeYProject/key/releases/tag/KeY-3.0.0) 
...
[Java Records](https://github.com/KeYProject/key/pull/3758)) are will automatically reduced to regular Java classes with generated methods and contracts, incl. support for [compact constructors](https://github.com/KeYProject/key/pull/3905). [TextBlockLiterals](https://github.com/KeYProject/key/pull/3796)) (`"""..."""`), 
[multi-catch-clauses in try-statements](https://github.com/KeYProject/key/pull/3919) and 
[try-with-resources](https://github.com/KeYProject/key/pull/3918) are similar handled. 
...
A [new parser](https://github.com/KeYProject/key/pull/3021) replaced the hand-written proof script parser, resulting in the earlier detection of syntax errors. It also makes proof scripts first-class citizens in KeY files - instead of embedding scripts in string literals. User can now write them directly: `\proofscript { andLeft; andRight; rule a ==> b; }`. The [introduction of higher-order commands](https://github.com/KeYProject/key/pull/3654) allows to easily extend the proof scripts with complex behavior easily. Proof commands can receive blocks of sub-commands like `integer-split "<0" : { auto; } "=0" : { instantiate; } ">0" : { tryclose; }` or even `if "cond" { .. then ..} {.. else ..}`.
[A simplie AST](https://github.com/KeYProject/key/pull/3587) is now available for proof scripts (see `ScriptCommandAst` class) abstracting the script sources into a simple record containing command name, positional/keyword arguments, sub-commands, and location information enabling JML script integration. 
Finally, [JML scripts have landed](https://github.com/KeYProject/key/pull/3657) in KeY, bringing proof scripts into JML comments, allowing scripts to be written directly alongside specifications and executed during automatic verification—for example, embedding proof guidance within `/*@ ... @*/` JML annotations so that proof node localization becomes natural and users can leverage JML expressions without leaving the specification language. Together, these changes transform proof scripts from an afterthought into an integrated, type-safe, and expressive component of the KeY verification workflow.
...
We fitted KeY's type system to the upcoming generics support. [Add Polymorphic Sorts and Functions](https://github.com/KeYProject/key/pull/3652) were introduced using a syntax like `PSeq<[E]>` and `select<[int]>`, along with matching logic, support for polymorphic datatypes, and [co- and contravariance](https://github.com/KeYProject/key/pull/3841). [Sort depending functions were purged](https://github.com/KeYProject/key/pull/3773), e.g., `int::select`,  in favor of parametric functions with parametric function syntax (`select<[int]>`). [Sort aliases](https://github.com/KeYProject/key/pull/3778) are helping to avoid long sort names inthe logic, e.g., `\alias IntSet = Set<[int]>;`. 
...
* [Make `\sameUpdateLevel` the default](https://github.com/KeYProject/key/pull/3899)
  ...
  > We made `\sameUpdateLevel` default in all Taclets. This prevents a common user error, and increase the user experience.

* [Improved treatment of final fields](https://github.com/KeYProject/key/pull/3495)

  > The immutability of final fields is now handled via a "secondary" immutable heap. 
  > For example, specifiying the invariance of final fields in loop invariants
...
* [Better ImmutableList](https://github.com/KeYProject/key/pull/3816)
  ...
  > `ImmutableList` got an extensive overhaul and many operations are now non-copying in O(1). 
  > `ImmutableArray` is deprecated.
...
* [Removal of `key.core.symbolic_execution` and `key.core.proof_references` modules](https://github.com/KeYProject/key/pull/3910)
  > The modules for symbolic execution and proof references were removed w/o replacement b/c maintainability issues.
...
* [MetaSpace for documentation and origin](https://github.com/KeYProject/key/pull/3787)
  ...
  > `MetaSpace` is the companion of the namespaces, catching and gathering proof irrelevant information like documentation and origin of KeY entities. 
...
* [Renovation of the TestCase generation](https://github.com/KeYProject/key/pull/3388)
  ...
  > The test case generation was overlooked and refreshed to modern standard. For example, 
  > build files (`pom.xml`, `build.gradle`) are now generated for compiling and running the tests. 
  > Only JUnit 5 is support. TestNG and JUnit 4 are removed.
...
* [Feature: Version-based Configuration Paths](https://github.com/KeYProject/key/pull/3888)
  ...
  > Configurations are now versioned (e.g., `~/.key/v3.0/`) with read-only fallback to previous versions.
...
* [Directly create a Github issue inside of KeY (in the menu under "About")](https://github.com/KeYProject/key/pull/3736)
* [Sequents are translatable to Isabelle](https://github.com/KeYProject/key/pull/3514)
* [Assignments and If-statements became valid in the body of model method](https://github.com/KeYProject/key/pull/3571)
* [A basic Theory of Sets were introduced](https://github.com/KeYProject/key/pull/3777)
* [The soundiness dialog explicit explains the assumptions underlying the current proof and selected Taclet options](https://github.com/KeYProject/key/pull/3887)
...
Over **180 other Pull Requests** for minor features, 🐛 bug fixes and other changes are not mentioned, but not forgotten. [Please consult the generated changelog for a full view.](https://github.com/KeYProject/key/releases/tag/KEY-3.0.0).


## [2.13.0](https://github.com/KeYProject/key/releases/tag/KEY-2.13.0) (2025-08-XX)
