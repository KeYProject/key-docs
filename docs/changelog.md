...
Internally, KeY used recoder to parse Java which got stucked in the year 2009. With the transition to [JavaParser](https://github.com/javaparser/javaparser), we can finally look into the future. 
The current fork which supports ProofJava, SchemaJava, and Java 21+ in a unified grammar is here [wadoon/key-javaparser](https://github.com/wadoon/key-javaparser). 
[PR 3120](https://github.com/KeYProject/key/pull/3120) enables support for newer Java syntax: [We support for `var` variable declarations](https://github.com/KeYProject/key/pull/3821)), e.g., `var x = 0;`. 
[Java Records](https://github.com/KeYProject/key/pull/3758)) are will automatically reduced to regular Java classes with generated methods and contracts, incl. support for [compact constructors](https://github.com/KeYProject/key/pull/3905). [TextBlockLiterals](https://github.com/KeYProject/key/pull/3796)) (`"""..."""`) are similar handled. 
...
Proof Scripts got a larger update for 3.0 extending their capabilities. PR [#3021](https://github.com/KeYProject/key/pull/3021) replaced the hand-written proof script parser with ANTLR4-based KeYParser.g4, resulting in the earlier detection of syntax errors. It also makes proof scripts first-class citizens in KeY files—instead of embedding scripts in string literals, users can now write them directly: `\proofscript { andLeft; andRight; rule a ==> b; }`. The [introduction of higher-order commands](https://github.com/KeYProject/key/pull/3654) allows to easily extend the proof scripts with complex behavior easily. Now proof commands can receive blocks of sub-commands, such as `try { auto; }` or `onAll { hide \f(x); }` and execute these commands accordingly to their internal rules. 
This goes even further allowing script blocks to appear as regular arguments anywhere (not just at command ends), enabling constructs like `integer-split "<0" : { auto; } "=0" : { instantiate; } ">0" : { tryclose; }` or even `if "cond" { .. then ..} {.. else ..}`.
[PR #3587](https://github.com/KeYProject/key/pull/3587) introduced a `ScriptCommandAst` class that abstracts script sources into a simple record containing command name, positional/keyword arguments, sub-commands, and location information, freeing the `ProofScriptEngine` from ANTLR dependencies and enabling JML script integration. 
Finally, [PR #3657](https://github.com/KeYProject/key/pull/3657) brought proof scripts into JML comments, allowing scripts to be written directly alongside specifications and executed during automatic verification—for example, embedding proof guidance within `/*@ ... @*/` JML annotations so that proof node localization becomes natural and users can leverage JML expressions without leaving the specification language. Together, these changes transform proof scripts from an afterthought into an integrated, type-safe, and expressive component of the KeY verification workflow.
...
KeY's modularization advanced.: [PR #3578](https://github.com/KeYProject/key/pull/3578) moved fundamental prover infrastructure—including sequents, proof objects, goals, rules, and the prover main loop—into the `ncore` module [1]. **PR #3640** ([Modularization: InfFlow and WD as separate modules](https://github.com/KeYProject/key/pull/3640)) extracted Information Flow and Well-Definedness functionality from the monolithic `key.core` into dedicated submodules (`key.core.infflow` and `key.core.wd`), using Profile-based enforcement to swap standard rules with feature-specific variants [2]. **PR #3783** ([move Taclet options of WD from key.core to key.core.wd](https://github.com/KeYProject/key/pull/3783)) completed the WD modularization by relocating remaining taclet option declarations to `key.core.wd`, ensuring WD-specific configuration is properly encapsulated [3]. Together, these changes transform KeY from a monolithic architecture into a maintainable, extensible system where verification features can be added as separate modules.
...
* [Set the Java Release Version to 21](https://github.com/KeYProject/key/pull/3522)
  ...
  > Modules are not supported yet, blocked package uniqueness and stringtemplate4.

* [Removal of Triple, and Quadruple](https://github.com/KeYProject/key/pull/3529)

  > The classes `Triple<A,B,C>` and `Quadruple<A,B,C,D>` are gone for maintainability and readability.
  > Please use Java records instead. 

* [Migration to java.nio.Path](https://github.com/KeYProject/key/pull/3618)

  > KeY uses `Path` now internally. `Path` is more plattform sensitive, you can experience difference on your code between Windows/Linux. 

* [Cleanup: Removal of key.api package](https://github.com/KeYProject/key/pull/3550)

  > The `key.api` package was deleted. This package was started once for a clear interface for the ProofScriptDebugger, 
  > but never reached the maturity and usefulness. Useful parts have been into the other classes. 
...
* [Feature: Version-based Configuration Paths](https://github.com/KeYProject/key/pull/3888)
  ...
  > **PR #3888** ([Feature: Version-based Configuration Paths](https://github.com/KeYProject/key/pull/3888)) introduced versioned configuration directories (e.g., `~/.key/v3.0/`) with fallback logic to previous versions [6]. 
